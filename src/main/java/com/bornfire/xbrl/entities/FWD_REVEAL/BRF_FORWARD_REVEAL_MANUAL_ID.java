package com.bornfire.xbrl.entities.FWD_REVEAL;

import java.io.Serializable;

import java.util.Objects;

import javax.persistence.Embeddable;


@Embeddable
public class BRF_FORWARD_REVEAL_MANUAL_ID implements Serializable {
	
	private static final long serialVersionUID = 1L;
		private String	num_operation;
        private String  report_date;
		public String getNum_operation() {
			return num_operation;
		}
		public void setNum_operation(String num_operation) {
			this.num_operation = num_operation;
		}
		public String getReport_date() {
			return report_date;
		}
		public void setReport_date(String report_date) {
			this.report_date = report_date;
		}
		public static long getSerialversionuid() {
			return serialVersionUID;
		}
		@Override
		public int hashCode() {
			return Objects.hash(num_operation, report_date);
		}
		@Override
		public boolean equals(Object obj) {
			if (this == obj)
				return true;
			if (obj == null)
				return false;
			if (getClass() != obj.getClass())
				return false;
			BRF_FORWARD_REVEAL_MANUAL_ID other = (BRF_FORWARD_REVEAL_MANUAL_ID) obj;
			return Objects.equals(num_operation, other.num_operation) && Objects.equals(report_date, other.report_date);
		}
		public BRF_FORWARD_REVEAL_MANUAL_ID(String num_operation, String report_date) {
			super();
			this.num_operation = num_operation;
			this.report_date = report_date;
		}
		public BRF_FORWARD_REVEAL_MANUAL_ID() {
			super();
			// TODO Auto-generated constructor stub
		}
		
        
        
        
        
        

		}
	
	



