...
public interface BRF2_MAPPING_REPO extends JpaRepository<BRF2_MAPPING_ENTITY, String>, BRF_MAPPING {
	@Query(value = "select * from BRF2_MAPPING_TABLE", nativeQuery = true)
	List<BRF2_MAPPING_ENTITY> getreport();
	...
	@Query(value = "select * from BRF2_MAPPING_TABLE where foracid=?1", nativeQuery = true)
	BRF2_MAPPING_ENTITY findByIDoo(String foracid);

	@Query(value = "SELECT * FROM BRF2_MAPPING_TABLE", nativeQuery = true)
	List<BRF2_MAPPING_ENTITY> getLiist2();
...
//	Query For Filter 
	@Query(value = "SELECT s.cust_id as custid, s.foracid as foracid, s.acct_name as acctname, s.report_label_1 as reportlabel1, "
			...
			+ "FROM BRF2_MAPPING_TABLE s WHERE s.SCHM_CODE = :scheme_code and s.GL_SUB_HEAD_CODE = :GL_SUB_HEAD_CODE and "
	...
	@Query(value = "SELECT s.cust_id as custid, s.foracid as foracid, s.acct_name as acctname, s.report_label_1 as reportlabel1, "
			+ "s.gl_sub_head_code as glsubheadcode, s.schm_code as schmcode " + "FROM BRF2_MAPPING_TABLE s WHERE "
	...
	@Query(value = "SELECT DISTINCT GL_CODE FROM BRF2_MAPPING_TABLE ORDER BY GL_CODE", nativeQuery = true)
	List<String> detail1();

	@Query(value = "SELECT DISTINCT CUST_ID, FORACID FROM BRF2_MAPPING_TABLE ORDER BY CUST_ID ASC,FORACID ASC", nativeQuery = true)
	List<String[]> getListDropDown(String cust_id, String foracid);

	@Query(value = "SELECT DISTINCT SCHM_CODE FROM BRF2_MAPPING_TABLE ORDER BY SCHM_CODE", nativeQuery = true)
	List<String> detail5();

	@Query(value = "SELECT DISTINCT GL_SUB_HEAD_CODE FROM BRF2_MAPPING_TABLE ORDER BY GL_SUB_HEAD_CODE", nativeQuery = true)
	List<String> detail6();
