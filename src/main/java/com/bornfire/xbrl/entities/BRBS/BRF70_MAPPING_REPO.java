...
public interface BRF70_MAPPING_REPO extends JpaRepository<BRF70_MAPPING_ENTITY, String>, BRF_MAPPING {
	@Query(value = "select * from BRF70_MAPPING_TABLE", nativeQuery = true)
	List<BRF70_MAPPING_ENTITY> getreport();

	@Query(value = "select * from BRF70_MAPPING_TABLE where report_name_1=?1", nativeQuery = true)
	List<BRF70_MAPPING_ENTITY> getdesc(String report_name_1);
	...
	@Query(value = "select s.cust_id as custid, s.foracid as foracid, s.acct_name as acctname, s.report_name_1 as reportname1, "
	        ...
	        + "from BRF70_MAPPING_TABLE s where s.report_label_1=?1", nativeQuery = true)
	List<BRF_PRODUCT_MAPPINGREPO> getproduct(String report_label_1);


	@Query(value = "select * from BRF70_MAPPING_TABLE", nativeQuery = true)
	List<BRF70_MAPPING_ENTITY> Mapping1();

	@Query(value = "select * from BRF70_MAPPING_TABLE where foracid=?1", nativeQuery = true)
	BRF70_MAPPING_ENTITY findByIDoo(String foracid);

	@Query(value = "SELECT * FROM BRF70_MAPPING_TABLE", nativeQuery = true)
	List<BRF70_MAPPING_ENTITY> getLiist2();

	@Query(value = "SELECT * FROM BRF70_MAPPING_TABLE WHERE REPORT_NAME_1=?1", nativeQuery = true)
	List<BRF70_MAPPING_ENTITY> getLiist(String REPORT_NAME_1);
	...
	@Query(value = "select s.cust_id as custid,s.foracid as foracid , s.acct_name as acctname , s.report_name_1 as reportname1 , "
			...
			+ " from BRF70_MAPPING_TABLE s  WHERE s.report_label_1 is not null AND s.report_addl_criteria_1 IS NOT NULL"
	...
	@Query(value = "SELECT  s.cust_id as custid,s.foracid as foracid , s.acct_name as acctname , s.report_name_1 as reportname1 ,"
			+ " s.report_label_1 as reportlabel1 , s.gl_sub_head_code as glsubheadcode , s.schm_code as schmcode,"
			+ " s.report_addl_criteria_1 as reportaddlcriteria1,s.report_addl_criteria_2 as reportaddlcriteria2,s.report_addl_criteria_3 as reportaddlcriteria3"
			+ " FROM BRF70_MAPPING_TABLE s WHERE s.report_label_1 IS  NULL OR s.report_addl_criteria_1 IS NULL", nativeQuery = true)
	List<BRF_MAPPING_PROPERTY> genUnMapped();

	
	@Query(value = "SELECT * FROM BRF70_MAPPING_TABLE WHERE SCHM_CODE = :scheme_code and GL_SUB_HEAD_CODE = :GL_SUB_HEAD_CODE", nativeQuery = true)
...
//	Query For Filter 
	@Query(value = "SELECT s.cust_id as custid, s.foracid as foracid, s.acct_name as acctname, s.report_label_1 as reportlabel1, "
			...
			+ "FROM BRF70_MAPPING_TABLE s WHERE s.SCHM_CODE = :scheme_code and s.GL_SUB_HEAD_CODE = :GL_SUB_HEAD_CODE and "
	...
	@Query(value = "SELECT s.cust_id as custid, s.foracid as foracid, s.acct_name as acctname, s.report_label_1 as reportlabel1, "
			+ "s.gl_sub_head_code as glsubheadcode, s.schm_code as schmcode " + "FROM BRF70_MAPPING_TABLE s WHERE "
	...
	@Query(value = "SELECT DISTINCT GL_CODE FROM BRF70_MAPPING_TABLE ORDER BY GL_CODE", nativeQuery = true)
	List<String> detail1();

	@Query(value = "SELECT DISTINCT CUST_ID, FORACID FROM BRF70_MAPPING_TABLE ORDER BY CUST_ID ASC,FORACID ASC", nativeQuery = true)
	List<String[]> getListDropDown(String cust_id, String foracid);

	@Query(value = "SELECT DISTINCT SCHM_CODE FROM BRF70_MAPPING_TABLE ORDER BY SCHM_CODE", nativeQuery = true)
	List<String> detail5();

	@Query(value = "SELECT DISTINCT GL_SUB_HEAD_CODE FROM BRF70_MAPPING_TABLE ORDER BY GL_SUB_HEAD_CODE", nativeQuery = true)
	List<String> detail6();
