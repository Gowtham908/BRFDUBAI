...
import com.bornfire.xbrl.entities.BOB_RAM_ENTITY;
...
import com.bornfire.xbrl.entities.ECL_GLCODE_DESC;
import com.bornfire.xbrl.entities.ECL_GLCODE_DESC_rep;
import com.bornfire.xbrl.entities.ECL_INFOFLG;
import com.bornfire.xbrl.entities.ECL_INFO_REP;
import com.bornfire.xbrl.entities.ECL_MAPPING_COUNTRY_CODE;
import com.bornfire.xbrl.entities.ECL_MAPPING_COUNTRY_CODE_REP;
import com.bornfire.xbrl.entities.ECL_MAPPING_SUBPORT;
import com.bornfire.xbrl.entities.ECL_MAPPING_SUBPORT_REP;
import com.bornfire.xbrl.entities.ECL_PAYMENT_FREQUENCY;
import com.bornfire.xbrl.entities.ECL_PAYMENT_FREQUENCY_rep;
import com.bornfire.xbrl.entities.ECL_SCHEM_CODE;
import com.bornfire.xbrl.entities.ECL_SCHEM_REP;
import com.bornfire.xbrl.entities.FinUserProfile;
import com.bornfire.xbrl.entities.GenRefCodeMast;
import com.bornfire.xbrl.entities.Scorecard_DBR_Entity;
...
import com.bornfire.xbrl.entities.scorecard_LTV_Entity;
import com.bornfire.xbrl.entities.scorecard_LTV_repo;
...
import com.bornfire.xbrl.services.TransitionCountRepository;
...
public class XBRLRestController {
	...
	public String getmail(@RequestParam("user") String yearRange, @RequestParam("password") String password,
			@RequestParam("pop3Host") String pop3Host, @RequestParam("storeType") String storeType, Model md)
			throws SQLException, ParseException {
		return transitionCountRepository.calculateAndPrintTransitionCounts(yearRange);
		// return "Success";
	}
	...
	@GetMapping(value = "Getcentraltendrep")
	public List<BOB_RAM_ENTITY> Getcentraltendrep(@RequestParam String message2, @RequestParam String message22) {
	...
	public String uploadFilePO(@RequestParam("file") MultipartFile file, String screenId,
		...
		if (file.getSize() < 50000000) {
			...
			String msg = alertManagementServices.UploadPO(userid, file, userid, issue);
			return msg;
		} else {
	...
	@GetMapping("SubPortfolioCode")
	public List<ECL_MAPPING_SUBPORT> SubPortfolioCode() {
		System.out.println("Entering port");
		List<ECL_MAPPING_SUBPORT> response = subportrep.getsubport();
		return response;
	}

	@Autowired
	ECL_MAPPING_COUNTRY_CODE_REP countrycode;

	@GetMapping("/countrycode")
	public List<ECL_MAPPING_COUNTRY_CODE> countrycode() {
	...
	@Autowired
	ECL_GLCODE_DESC_rep glcoderep;

	@GetMapping("/glcodedesc")
	public List<ECL_GLCODE_DESC> getglcode() {
		System.out.println("Entering code");
		List<ECL_GLCODE_DESC> response = glcoderep.getglcode();
		System.out.println(response);
		return response;
	}

	@Autowired
	ECL_PAYMENT_FREQUENCY_rep payfreq_rep;

	@GetMapping("/paymentfrquncy")
	public List<ECL_PAYMENT_FREQUENCY> paymentfrquncy() {
		System.out.println("Entering code");
	...
	@Autowired
	ECL_INFO_REP inforep;

	@GetMapping("/infoflg")
	public List<ECL_INFOFLG> infoflg() {
		System.out.println("Entering code");
		List<ECL_INFOFLG> response = inforep.getINFO();
		System.out.println(response);
		return response;
	}

	@Autowired
	ECL_SCHEM_REP SCHEMREP;

	@GetMapping("/schemecode")
	@ResponseBody
	public List<ECL_SCHEM_CODE> schemecode() {
		System.out.println("Entering code");
		List<ECL_SCHEM_CODE> response = SCHEMREP.getSCHEM();
		System.out.println(response);
		for (ECL_SCHEM_CODE a : response)
	...
	/*-------scorecardLTV----*/
	@Autowired
	scorecard_LTV_repo ltv_repo;

	@GetMapping("/ltv")
	public List<scorecard_LTV_Entity> ltv() {
		System.out.println("Entering ltv");
		List<scorecard_LTV_Entity> response = ltv_repo.getLTD();
		System.out.println(response);
		return response;
	}
	...
	@ResponseBody
	public String auditschedulemodifysubmit(@RequestBody List<Scorecard_DBR_Entity> scorecard_DBR_Entities) {
	...
	@PostMapping("/submit")
	public String submitDBRList(@ModelAttribute("getDBRvalue") List<Scorecard_DBR_Entity> dbrList) {

		for (Scorecard_DBR_Entity dbr : dbrList) {
			// Save or update each DBR object
			dbr_repo.save(dbr);
		}

		String msg = "modified successfully";
		return msg;
	}
	...
	public ResponseEntity<String> tmtuploadxmlvaluesdatas(@RequestParam("files") List<MultipartFile> files,
	    ...
	    for (int i = 0; i < files.size(); i++) {
	        ...
	        try (InputStream stream = file.getInputStream()) {
	            ...
	            JAXBElement<FYItransactions.Document> jaxbElement = unMarshaller.unmarshal(reader, FYItransactions.Document.class);
	            FYItransactions.Document doc = jaxbElement.getValue();

	            String stmtIdentifier = doc.getBkToCstmrStmt().getStmt().getAcct().getId().getOthr().getStmtAccountIdentifier();
	         ...
	         //       logger.info("Key Value to check : "+ key.toString() );
	            ...
	            if (!duplicateEntries.isEmpty()) {
	                if (!duplicates.contains(fileName)) duplicates.add(fileName);
		...
		if (reportNames != null && !reportNames.isEmpty()) {
			...
			if (!returnrecord.isEmpty()) {
				try (Workbook workbook = new XSSFWorkbook()) {
					...
					// Set up headers for file download
					HttpHeaders headers1 = new HttpHeaders();
					headers1.add(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=LAC_TTUM_"+ formattedYesterdayDate.replace(".","-")+".xlsx");
					ByteArrayResource resource = new ByteArrayResource(outputStream.toByteArray());
