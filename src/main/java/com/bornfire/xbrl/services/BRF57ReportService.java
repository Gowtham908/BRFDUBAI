...
import com.bornfire.xbrl.entities.BRBS.BRF057ServiceRepo;
import com.bornfire.xbrl.entities.BRBS.BRF57_ENTITY;
...
import com.bornfire.xbrl.entities.BRBS.BRF57_DETAIL_ENTITY;
import com.bornfire.xbrl.entities.BRBS.BRF57_ARCHIVENTITY;
import com.bornfire.xbrl.entities.BRBS.BRF57_DETAIL_ENTITY;
import com.bornfire.xbrl.entities.BRBS.BRF57_DetailRepo;
import com.bornfire.xbrl.entities.BRBS.BRF57_ENTITY;
import com.bornfire.xbrl.entities.BRBS.BRF86_DETAIL_ENTITY;
...
public class BRF57ReportService {
	private static final Logger logger = LoggerFactory.getLogger(BRF57ReportService.class);
	...
	public String preCheckBRF057(String reportId, String fromdate, String todate) {
		...
		try {
			...
			Long dtlcnt = (Long) hs.createQuery("select count(*) from BRF57_ENTITY a where a.report_date=?1")
					.setParameter(1, dt9).getSingleResult();

			if (dtlcnt > 0) {
				logger.info("Getting No of records in Mod table :" + reportId);
				Long modcnt = (Long) hs.createQuery("select count(*) from BRF57_ENTITY a").getSingleResult();
				if (modcnt > 0) {
					msg = "success";
				}
			} else {
	...
	public ModelAndView getBRF057View(String reportId, String fromdate, String todate, String currency, String dtltype,
			Pageable pageable) {
		...
		List<BRF57_ENTITY> T1rep = new ArrayList<BRF57_ENTITY>();


		List<BRF57_ENTITY> T1Master = new ArrayList<BRF57_ENTITY>();
		...
		try {
			...
			T1Master = hs.createQuery("from BRF57_ENTITY a where a.report_date = ?1 ", BRF57_ENTITY.class)
	...
	public ModelAndView getBRF057Details(String reportId, String fromdate, String todate, String currency,
		...
		if (dtltype.equals("report")) {
			if (!filter.equals("null")) {
				qr = hs.createNativeQuery(
						"select * from BRF57_DETAILTABLE  a where report_date = ?1 and report_label_1=?2");

				qr.setParameter(2, filter);

			} else {
				qr = hs.createNativeQuery("select * from BRF57_DETAILTABLE a where report_date = ?1");

			}
		} else {
			qr = hs.createNativeQuery("select * from BRF57_DETAILTABLE  where report_date = ?1");
		}
		...
		List<BRF57_DETAIL_ENTITY> T1Master = new ArrayList<BRF57_DETAIL_ENTITY>();

		try {
			T1Master = hs.createQuery("from BRF57_DETAIL_ENTITY a where a.report_date = ?1", BRF57_DETAIL_ENTITY.class)
					.setParameter(1, df.parse(todate)).getResultList();
		} catch (ParseException e) {
		...
		for (Object[] a : result) {
			...
			BRF57_DETAIL_ENTITY py = new BRF57_DETAIL_ENTITY(cust_id, foracid, act_balance_amt_lc, eab_lc, acct_name,
		...
		mv.setViewName("RR"+"/"+"BRF57::reportcontent");
	...
	public File getFileBRF057(String reportId, String fromdate, String todate, String currency, String dtltype,
			...
			if(!filetype.equals("BRF")) {
				...
				try {
					...
					if (filetype.equals("detailexcel")) {
						if (dtltype.equals("report")) {
							...
							jasperFile = this.getClass().getResourceAsStream("/static/jasper/BRF57_DETAIL.jrxml");
						} else {
							jasperFile = this.getClass().getResourceAsStream("/static/jasper/BRF57_DETAIL.jrxml");
						}
						...
						if (dtltype.equals("report")) {
							...
							jasperFile = this.getClass().getResourceAsStream("/static/jasper/BRF57.jrxml");
						} else {
							jasperFile = this.getClass().getResourceAsStream("/static/jasper/BRF57.jrxml");
						}
					}
				...
				return outputFile;
					...
					if(T1Master.size() == 1) {
						...
						for(BRF57_ENTITY BRF57 : T1Master ) {
					
					File Responsecamt = new File(env.getProperty("output.exportpathtemp")+"011-BRF-057-AT.xls");
		        	...
		        	Workbook workbook = WorkbookFactory.create(Responsecamt);
		        	 ...
		        	 FileOutputStream fileOut = new FileOutputStream(env.getProperty("output.exportpathfinal")+"011-BRF-057-A.xls");
				...
				outputFile = new File(env.getProperty("output.exportpathfinal")+"011-BRF-057-A.xls");
	...
	public ModelAndView getArchieveBRF057View(String reportId, String fromdate, String todate, String currency,
	...
	public String detailChanges57(BRF57_DETAIL_ENTITY detail, String foracid, String report_addl_criteria_1,
		...
		try {
			...
			Session hs = sessionFactory.getCurrentSession();
			Optional<BRF57_DETAIL_ENTITY> BRF57detail =brf57_DetailRepo.findById(foracid);

			if (BRF57detail.isPresent()) {
				BRF57_DETAIL_ENTITY BRFdetail = BRF57detail.get();
				...
				if (BRFdetail.getReport_label_1().equals(report_label_1)
					...
					brf57_DetailRepo.save(BRFdetail);
...
//						hs.saveOrUpdate(detail);
	...
	public ModelAndView getArchieveBRF57View(String reportId, String fromdate, String todate, String currency,
			String dtltype, Pageable pageable) {
	...
	public ModelAndView ARCHgetBRF57currentDtl(String reportId, String fromdate, String todate, String currency,
		...
		if (dtltype.equals("report")) {
			if (!filter.equals("null")) {
				...
				qr = hs.createNativeQuery("select * from BRF57_ARCHIVTABLE a where report_date=?1 and report_label_1=?2");
				qr.setParameter(1, df.parse(todate));
				qr.setParameter(2, filter);

			} else {
				System.out.println("2");
				qr = hs.createNativeQuery("select * from BRF57_ARCHIVTABLE");

			}
		} else {
			System.out.println("3");
			qr = hs.createNativeQuery("select * from BRF57_ARCHIVTABLE  where report_date = ?1");
		}
		 ...
		 */
		List<BRF57_ARCHIVENTITY> T1Master = new ArrayList<BRF57_ARCHIVENTITY>();

		try {
			System.out.println("Values entered");
			T1Master = hs.createQuery("from BRF57_ARCHIVENTITY a where a.report_date = ?1", BRF57_ARCHIVENTITY.class)
					.setParameter(1, df.parse(todate)).getResultList();
		} catch (ParseException e) {
		...
		for (Object[] a : result) {
			...
			BRF57_ARCHIVENTITY py = new BRF57_ARCHIVENTITY(cust_id, foracid, act_balance_amt_lc, eab_lc, acct_name,
					acct_crncy_code, gl_code, gl_sub_head_code, gl_sub_head_desc, country_of_incorp, cust_type,
					schm_code, schm_type, sol_id, acid, segment, sub_segment, sector, sub_sector, sector_code, group_id,
					constitution_code, country, legal_entity_type, constitution_desc, purpose_of_advn, hni_networth,
					turnover, bacid, report_name_1, report_label_1, report_addl_criteria_1, report_addl_criteria_2,
		...
		mv.setViewName("RR" + "/" + "BRF57ARCH::reportcontent");
